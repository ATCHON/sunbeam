...
def write_fastq(genome_segment, locs, sample_name, file_fp, rp_suffix=None):
	...
	file1 = gzip.open(f'{file_fp}/PCMP_{sample_name}_R1.fastq.gz', 'wt')
	for i, seq in enumerate(seqs1):
		header = f"@D00728:28:C9W1KANXX:{str(i)}"
		if rp_suffix:
			header += rp_suffix[0]
		file1.write("%s\n" % header)
		file1.write("%s\n" % seq)
	...
	file2 = gzip.open(f'{file_fp}/PCMP_{sample_name}_R2.fastq.gz', 'wt')
...
def write_random_fastq(sample_name, file_fp, numreads=4, readlen=100):
	...
	randread = lambda: ''.join(
	    [dna[random.randint(0,
	                        len(dna) - 1)] for _ in range(readlen)])
	for r in (1,2):
		with gzip.open(f'{file_fp}/PCMP_{sample_name}' + '_R%s.fastq.gz' % r, 'wt') as f:
...
def generate_dummyfastq(rootpath, fastq_fn = "data_files", rp_suffix=None):
	...
	raw_fp = f'{rootpath}/raw'

	# read in the 10k fragments
	with open(f'{raw_fp}/GCF_Ecoli_10k_genomic.fna') as f:
		lines_e = f.read().splitlines()

	with open(f'{raw_fp}/GCF_Bfragilis_10k_genomic.fna') as f:
		lines_b = f.read().splitlines()

	## read in 1000 random start position and fragment length
	with open(f'{raw_fp}/index_10k.txt') as f:
